...
\begin{frame}{Fabricación de Circuitos Integrados}
    \begin{columns}
        ...
        \begin{tikzpicture}[scale=0.55, transform shape]
            % IC Layers Diagram
            \draw[fill=kit-blue20, draw=mnnwInk] (-2, 0) rectangle (2, 0.4);
            \node[font=\tiny] at (0, 0.2) {Sustrato de Silicio};

            \draw[fill=mnnwHair!20, draw=mnnwInk] (-1.8, 0.4) rectangle (-1.2, 0.6);
            \draw[fill=mnnwHair!20, draw=mnnwInk] (1.2, 0.4) rectangle (1.8, 0.6);
            \node[font=\tiny, anchor=west] at (2.1, 0.5) {Transistores};

            \foreach \h in {0.8, 1.4, 2.0} {
                \draw[fill=kit-orange50, draw=mnnwInk] (-1.5, \h+0.2) rectangle (1.5, \h+0.4);
                \draw[fill=kit-orange50!80, draw=mnnwInk] (-0.8, \h) rectangle (-0.6, \h+0.2);
                \draw[fill=kit-orange50!80, draw=mnnwInk] (0.6, \h) rectangle (0.8, \h+0.2);
            }
            \node[font=\tiny, anchor=west] at (2.1, 1.6) {Capas de Metal};
        \end{tikzpicture}
...
\begin{frame}{Productividad vs. \textit{Overlay}}
    \begin{columns}
        ...
        \begin{blueblock}{Overlay (nm)}
            Alineación entre las múltiples capas de un IC (\cite{Schmidt2012_UltraPrecision}).
        \end{blueblock}
        ...
        \vspace{0.2cm}
        El tiempo de asentamiento reduce la productividad, pero es vital para mitigar las vibraciones que degradan el \textit{overlay} (\cite{Butler2011_PositionControl}).
        %Las especificaciones exigen trayectorias que minimicen simultáneamente las vibraciones residuales y el tiempo de asentamiento.
        ...
        \begin{tikzpicture}[scale=0.75, transform shape]
            ...
            \begin{scope}[shift={(0,3.5)}]
                \node[font=\tiny\bfseries, anchor=south] at (1.5, 2.5) {Vista Aerea};
...
\begin{frame}{Antecedentes}
    Investigadores y fabricantes han abordado la mitigación de vibraciones y la mejora del posicionamiento mediante diversas estrategias:
    
    \vspace{0.3cm}
    \begin{itemize}
        \item \textbf{Control Activo:} Controladores no lineales (ej. \textit{Hybrid Integrator-Gain Systems}) para contrarrestar el retraso de fase y reducir el sobretiro (\cite{Heertjes2025_NonlinearControl}).
        \item \textbf{Arquitecturas Dinámicas:} Uso de configuración de triple etapas (\textit{fine stages}) (\cite{Al-Rawashdeh2022_Caracterization}).
        \item \textbf{\textit{Input Shaping}:} Trayectorias de referencia acotadas en el \textit{jerk} para evitar la excitación de frecuencias resonantes estructurales conocidas (\cite{Al-Rawashdeh2023_Trajectories}).
    \end{itemize}
    \vspace{0.3cm}
    Los algoritmos de planeación suelen estar aislados del contexto del proceso (el patrón), optimizando métricas servo-mecánicas (MSE) sin considerar el impacto morfológico en el rendimiento (\textit{yield}).
\end{frame}

\begin{frame}{Hipótesis y Objetivos}
    \begin{purpleblock}{Hipótesis}
        La integración de una red neuronal convolucional (CNN) para la clasificación de mapas de defectos permite optimizar de forma predictiva los parámetros cinemáticos de una trayectoria de cuarto orden, maximizando el rendimiento (\textit{yield}) sin penalizar la productividad (\textit{throughput}) del sistema litográfico.
    \end{purpleblock}

    \vspace{0.1cm}
    \textbf{Objetivos:}
    \begin{enumerate}
        \item Desarrollar una simulación (\textit{digital twin}) basado en un modelo multi-cuerpo.
        \item Generar analíticamente trayectorias \textit{step-and-scan} acotadas en derivadas superiores (acotado en \textit{snap}).
        \item Entrenar una CNN para la clasificación de defectos utilizando datos industriales (WM-811K).
        \item Utilizar la CNN como función de costo dinámica para encontrar perfiles de aceleración óptimos.
    \end{enumerate}
\end{frame}
...
\begin{frame}{Dinámica}
    \begin{itemize}
        \item Limitamos el movimiento $f_{i_{j_x}}$, $f_{i_{j_y}}$ y la rotación planar $\theta_z$.
...
\begin{frame}{Simulación}
    \begin{columns}
        \column{.6\textwidth}
        Implementación del modelo en \textbf{MuJoCo} (\cite{todorov2012mujoco}) usando los parámetros propuestos en \cite{Al-Rawashdeh2022_Caracterization}.
        \begin{center}
          \includegraphics[width=0.5\linewidth]{../fullThesis/pictures/scanning_3d.png}
        \end{center}
        \column{.4\textwidth}
        \begin{center}
...
\begin{frame}{Mapas de Defectos}
    \begin{columns}[T]
        \column{.6\textwidth}
        \begin{itemize}
            \item Conjunto de datos \textbf{WM811K} con 811,457 mapas de defectos de obleas de manufactura de alto volumen (\cite{Wu2015_WM811K}).
            \item Seleccionamos un subconjunto de 172,950 muestras etiquetadas en 9 clases de defectos de manufactura (como \textit{Center}, \textit{Edge-Loc}, \textit{Scratch}, \textit{Donut}).
            \item Nos permite relacionar parametros de diseño con las variables del proceso.
        \end{itemize}
        \column{.4\textwidth}
        \begin{center}
...
\begin{frame}{Arquitectura CNN}
    \begin{center}
        ...
        \begin{tikzpicture}[scale=1, transform shape]
            ...
            % input image
            \node[canvas is zy plane at x=0] (temp) at (-1.5,0,0) {\includegraphics[width=6cm,height=6cm]{pictures/mapa.png}};
            
            % conv1
            \pic[shift={(0,0,0)}] at (0,0,0) {RightBandedBox={name=cr1,caption=Conv1,%
                    xlabel={{"16","16"}},zlabel=64,fill=\ConvColor,bandfill=\ConvReluColor,%
                    height=32,width={2,2},depth=32}};
            %pool1
            \pic[shift={(0,0,0)}] at (cr1-east) {Box={name=p1,%
                    fill=\PoolColor,opacity=0.5,height=24,width=1,depth=24}};
            
            % conv2
            \pic[shift={(1.2,0,0)}] at (p1-east) {RightBandedBox={name=cr2,caption=Conv2,%
                    xlabel={{"32","32"}},zlabel=32,fill=\ConvColor,bandfill=\ConvReluColor,%
                    height=24,width={3,3},depth=24}};
            %pool2
            \pic[shift={(0,0,0)}] at (cr2-east) {Box={name=p2,%
                    fill=\PoolColor,opacity=0.5,height=16,width=1,depth=16}};
            
            % conv3
            \pic[shift={(1.2,0,0)}] at (p2-east) {RightBandedBox={name=cr3,caption=Conv3,%
            ...
            % fc1
            \pic[shift={(1.8,0,0)}] at (p3-east) {RightBandedBox={name=fc1,caption=FC1,%
                    xlabel={{"1",""}},zlabel=128,fill=\FcColor,bandfill=\FcReluColor,%
                    height=3,width=3,depth=20}};
            
            % fc2 (Output)
            \pic[shift={(1.5,0,0)}] at (fc1-east) {RightBandedBox={name=fc2,caption=FC2,%
                    xlabel={{"1",""}},zlabel=9,fill=\FcColor,bandfill=\FcReluColor,%
                    height=3,width=3,depth=6}};
            
            % softmax
            \pic[shift={(0.8,0,0)}] at (fc2-east) {Box={name=softmax,caption=Softmax,%
                    xlabel={{"","dummy"}},zlabel=9,opacity=0.8,fill=\SoftmaxColor,%
                    height=3,width=1.5,depth=6}};
            
            % Connections
            \draw [connection]  (p1-east)        -- node {\midarrow} (cr2-west);
            \draw [connection]  (p2-east)        -- node {\midarrow} (cr3-west);
            \draw [connection]  (p3-east)        -- node {\midarrow} (fc1-west);
    ...
    \begin{tabular}{ll@{\hspace{1cm}}ll}
        ...
        \midrule
        Entrada & $64 \times 64$ (Reescalado \textit{nearest}) & Optimización & Adam ($\text{lr}=10^{-3}$), 5 Epochs \\
        Arquitectura & 3$\times$Conv, 2$\times$FC (16, 32, 64) & \textbf{Precisión (Accuracy)} & \textbf{96.88\%} \\
        Función de Pérdida & CrossEntropy & \textbf{Validación} & Mapas simulados (MuJoCo) \\
        \bottomrule
    \end{tabular}
\end{frame}
...
\begin{frame}{Trayectorias de Alto Orden}
    \begin{columns}
        \column{.58\textwidth}
        \begin{itemize}
            \item Trayectorias de referencia con derivadas de orden superior acotadas (\textit{snap} finito), planificadas mediante polinomios segmentados óptimos en tiempo (\cite{Lambrechts2003_Trajectories}).
            \item Perfiles (cuadrado, biselado) derivados analíticamente mediante modelado cinemático-dinámico con \textit{input shaping} (\cite{Al-Rawashdeh2023_Trajectories}).
            \item Mejora en el seguimiento de la trayectoria, lo que representa a su vez una disminución del error de sincronización (\textit{overlay}).
        \end{itemize}
        \column{.42\textwidth}
        \begin{center}
