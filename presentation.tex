...
\title[Mecátronica]{Optimización de Trayectorias de Orden Superior Informadas por el Rendimiento de la Oblea para Sistemas \textit{Step-and-Scan}}
...
\begin{frame}{Fabricación de Circuitos Integrados}
    \begin{itemize}
        \item La \textit{ley de Moore} impulsa a la industria de semiconductores a reducir de manera exponencial el rendimiento de los dispositivos electronicos.
        \item Este dramático incremento en el rendimiento, implica la constante reducción en el tamaño de los componentes discretos en un circuito integraod (IC).
        \item La impresión de los patrones que componen cada capa del circuito se realiza mediante la exposición de la \textit{foto-máscara} en el proceso de \textbf{fotolitografía}.
...
\begin{frame}[fragile]{El Proceso \textit{Step-and-Scan}}
    \begin{columns}
        ...
        \begin{itemize}
            ...
            \item Por lo tanto, el movimiento de la \textbf{reticula} y la \textbf{oblea} debe estar sincronizado en direcciones opuestas.
            \item Debido a la tasa de reducción del lente, la reticula debe moverse $4\times$ más rápido.
...
\begin{frame}{Productividad vs. \textit{Overlay}}
    \begin{blueblock}{Overlay (nm)}
        Puesto que un solo IC consiste de múltiples capas que interactuan entre si, el \textit{overlay} es la desviación entre cada una que debe ser minimizada para asegurar su interconexión (\cite{Schmidt2012_UltraPrecision}).
    \end{blueblock}
    ...
    Es necesario encontrar trayectorias de referencia que minimizen el tiempo de asentamiento, y a su vez, la excitación de la resonancia estructural.
\end{frame}

\section{Modelo}

\begin{frame}{Sistema multicuerpo}
    El modelo, propuesto en \cite{Al-Rawashdeh2022_Caracterization}, esta compuesto por un marco base que relaciona la dinámica de tres cadenas seriales:
    \begin{itemize}
        \item \textbf{Cadena de la reticula ($j=1$):} desliza la fotomáscara bajo la fuente de iluminación.
        \item \textbf{Cadena de optica ($j=2$):} conecta el lente y el marco de metrología.
...
\begin{frame}[fragile]{Topologia}
    \begin{columns}
        ...
        \begin{itemize}
            ...
            \item La flexibilidad estructural de la estructura esta modelada mediante un sistema resorte-amortiguador ($K_{i_j}, C_{i_j}$) para cada cuerpo $i$ con $i+1$ en la cadena $j$.
        ...
        \begin{center}
          ...
          \node[lbl, anchor=east, text=mnnwMute] at (\xLeft-1.1-0.08,-0.18) {Floor};
...
\begin{frame}{Simulación}
    \begin{columns}
        ...
        \begin{itemize}
            ...
            \item Permite ejecutar las trayectorias, y seguir el error de posicionamiento a través del promedio móvil (MA) y la desviación estandar móvil (MSD).
            \item Actualmente modelamos los parametros propuestos en \cite{Al-Rawashdeh2022_Caracterization}.
...
\begin{frame}{Trayectorias de Alto Orden}
    \begin{columns}
        \column{.6\textwidth}
        \begin{itemize}
            \item Trayectorias de referencias con derivadas de orden superior acotadas (\textit{snap} finito).
            \item Reducción de la excitación de modos estructurales de bajo amortiguamiento.
            \item Mejora en el seguimiento de la trayectoria, que representa a su vez una disminución del error de sincronización (\textit{overlay}) (\cite{Al-Rawashdeh2023_Trajectories}).
        \end{itemize}
        \column{.4\textwidth}
        \begin{center}
          \resizebox{0.75\linewidth}{!}{
          \begin{tikzpicture}[
            ...
            %   \draw[square] (0, 0.6) -- (0.3, 0.6) node[right, font=\tiny] {Square};
            %   \draw[fillet] (0, 0.3) -- (0.3, 0.3) node[right, font=\tiny] {Fillet};
            %   \draw[meander] (0, 0) -- (0.3, 0) node[right, font=\tiny] {Meander};
            % \end{scope}
...
\begin{frame}{Siguientes pasos...}
    \begin{itemize}
        ...
        \item Integrar un estimador de parametros para el controlador.
