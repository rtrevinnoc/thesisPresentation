...
\begin{frame}{Fabricación de Circuitos Integrados}
    \begin{columns}
        ...
        \begin{tikzpicture}[scale=0.55, transform shape]
            ...
            \draw[draw=mnnwInk, -Stealth, line width=0.8pt] (-1.0, 3.6) -- (-1.8, 3.6) node[left, font=\tiny] {};
            ...
            \begin{scope}
                ...
                \foreach \x in {-1.8,-1.2,...,1.8} {
                    \foreach \y in {-0.2, 0.2} {
                        \draw[draw=waferEdge!40, line width=0.1pt] (\x-0.2, \y-0.15) rectangle (\x+0.2, \y+0.15);
                    }
                }
            \end{scope}
            \draw[fill=none, draw=waferEdge] (0, 0) ellipse (2.2cm and 0.45cm);
            \draw[draw=mnnwInk, -Stealth, line width=0.8pt] (1.4, 0.5) -- (2.2, 0.5) node[right, font=\tiny] {};
            ...
            \draw[draw=mnnwSignal!80!black, line width=1.0pt, opacity=0.6] (0, 1.55) -- (0, 0.1);

            % DIE SCAN DETAIL (Side)
            \begin{scope}[shift={(4.2, 3.5)}]
                \draw[draw=mnnwInk, fill=white] (-0.8,-1.2) rectangle (0.8,1.2);
                % Exposed part (patterns)
                \fill[mnnwMute!40] (-0.8, 0.1) rectangle (0.8, 1.2);
                \foreach \h in {0.3, 0.6, 0.9} {
                    \draw[mnnwInk, line width=1.2pt] (-0.5, \h) -- (0.5, \h);
                }
                % Slit
                ...
                % Wafer movement arrow
                \draw[draw=mnnwInk, -Stealth, line width=1.2pt] (1.1, 0.5) -- (1.1, -0.5) node[midway, right, font=\fontsize{4pt}{4pt}\selectfont] {};
                \node[font=\tiny, anchor=north] at (0, -1.2) {};
            \end{scope}
        \end{tikzpicture}
        ...
        \begin{tikzpicture}[scale=0.55, transform shape]
            ...
            % \draw[fill=mnnwHair!40, draw=mnnwInk] (-0.8, 2.6) arc (180:0:0.8 and 0.6) -- cycle;
            % \node[font=\tiny, anchor=west] at (2.1, 2.8) {Bump};
        \end{tikzpicture}
...
\begin{frame}{Mapas de Defectos}
    \begin{columns}[T]
        \column{.57\textwidth}
        \begin{itemize}
            \item Base \textbf{WM811K}: 811,457 mapas de oblea (\cite{Wu2015_WM811K}).
            \item Entrenamiento: 172,950 muestras etiquetadas (9 clases).
        \end{itemize}
        ...
        \begin{tabular}{ll}
            \toprule
            \textbf{Parámetro} & \textbf{Valor / Resultado} \\
            \midrule
            Arquitectura & 3$\times$Conv, 2$\times$FC (Filters: 16, 32, 64) \\
            Entrada & 64 $\times$ 64 (Reescalado \textit{nearest}) \\
            Optimización & Adam (lr=1e-3), 5 Epochs \\
            \midrule
            \textbf{Precisión (Accuracy)} & \textbf{96.88\%} \\
            \textbf{Función de Pérdida} & CrossEntropy \\
            \textbf{Validación} & Mapas generados en MuJoCo \\
            \bottomrule
        \end{tabular}

        \column{.03\textwidth}
        \centering
        \vrule width 0.4pt height 0.65\textheight
