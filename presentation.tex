...
\begin{frame}[fragile]{La CNN como puente: clase $\to$ costo}
    ...
    \begin{tikzpicture}[scale=0.85, every node/.style={font=\footnotesize}]
        % ---- Panel 1: simulated map thumbnail ----
        \node[draw=mnnwInk, line width=0.5pt, rounded corners=2pt, minimum width=2.4cm, minimum height=2.4cm, fill=white] (map) at (0,0) {};
        % wafer disc
        ...
        \end{scope}
        % scratch defect line (red)
        \draw[kit-red100, line width=0.8pt] (-0.55, 0.35) -- (0.45, -0.40);
        \node[font=\small, anchor=south] at (0, 1.35) {mapa simulado};
        ...
        \begin{scope}[shift={(4.4,0)}]
            \node[font=\small, anchor=south] at (1.2, 1.35) {$P_c$ por clase};
            \def\barH{0.20}
            \def\unit{2.6}
            \foreach \name/\val/\col/\grp [count=\i] in {%
                ...
                \pgfmathsetmacro{\y}{(5-\i)*0.27}
                \ifnum\pdfstrcmp{\grp}{plaus}=0
                    \node[anchor=east, font=\scriptsize] at (0, \y) {\name};
                \else
                    \node[anchor=east, font=\scriptsize, text=mnnwMute] at (0, \y) {\name};
                \fi
                \fill[\col] (0.05, \y-\barH/2) rectangle ({0.05+\val*\unit}, \y+\barH/2);
                \pgfmathsetmacro{\pct}{int(\val*100)}
                \ifnum\pct>0
                    \node[anchor=west, font=\scriptsize] at ({0.10+\val*\unit}, \y) {\pct\,\%};
                \fi
            }
        \end{scope}

        % Arrow 2: weighted sum
        \draw[-Stealth, line width=0.9pt] (7.6, 0) -- (8.7, 0);
        \node[font=\footnotesize\bfseries, anchor=south] at (8.15, 0.08) {$\sum w_c P_c$};

        % ---- Panel 3: cost ----
        \node[draw=opticsEdge, line width=0.8pt, rounded corners=3pt, fill=opticsFlex,
              minimum width=2.6cm, minimum height=2.0cm, align=center, font=\small] (cost) at (10.1, 0)
              {$\mathcal{J}$ \\ \textbf{0.65\,\%} \\ {\scriptsize pérdida esperada}};
        \node[font=\small, anchor=south] at (10.1, 1.35) {costo};
    \end{tikzpicture}\\[0.30cm]
    {\footnotesize Sólo las clases inducibles por servo (\textit{none, scratch, edge-local, random}) contribuyen a $\mathcal{J}$. Pesos $w_c$ por clase: literatura (\cite{Kim2017_Market,Kim2019_Productivity,Yang2018_Yield}) o calibración.}
    \end{center}
\end{frame}
