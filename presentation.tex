...
\begin{frame}{Dinámica}
    ...
    16 cuerpos $\cdot$ $N_1{=}7$ retícula, $N_2{=}4$ óptica, $N_3{=}7$ oblea $\cdot$ 6 estados/cuerpo $\Rightarrow x\in\mathbb{R}^{96}$
    \end{center}
    ...
    \end{column}
    \begin{column}{0.50\textwidth}\scriptsize
        $\bar m_{i_j}$: masa agregada (Steiner)\\[1pt]
        $\bar C,\bar K$: enlace $(i{-}1){\to}i$\\[1pt]
        $\hat C,\hat K$: acoplamiento aguas abajo \;$\cdot$\; Casos frontera: tesis
    \end{column}
    \end{columns}
\end{frame}
...
\begin{frame}{Simulación --- resultados}
    ...
    \end{center}
    \begin{center}
        \includegraphics[width=0.46\linewidth,height=0.55\textheight,keepaspectratio]{pictures/sim_combined_y.png}\hfill%
        \includegraphics[width=0.46\linewidth,height=0.55\textheight,keepaspectratio]{pictures/scanning_3d.png}\\[0.05cm]
        {\footnotesize Error $e_{syn}$, MA y MSD (dir.\ Y) \quad$|$\quad Entorno MuJoCo}
    \end{center}
\end{frame}
...
\begin{frame}{Mapas de defectos y CNN}
    ...
    \begin{columns}[T]
        ...
        {\footnotesize \textbf{WM-811K} (\cite{Wu2015_WM811K}): 138\,360 obleas, 9 clases}

        \column{.58\textwidth}
        \centering
        \includegraphics[width=\linewidth,height=0.70\textheight,keepaspectratio]{pictures/cnn_confmat.png}\\[0.05cm]
