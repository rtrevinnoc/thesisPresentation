    ...
    \begin{columns}
        ...
        \begin{tikzpicture}[scale=0.75, transform shape]
            ...
            \begin{scope}[shift={(0,3.5)}]
                \node[font=\scriptsize\bfseries, anchor=south] at (1.5, 2.5) {Vista aérea};
            ...
            \begin{scope}[shift={(0, 1.2)}]
                ...
                \node[anchor=west, font=\scriptsize] at (2.6, 0.82) {Capa actual};
                \node[anchor=west, font=\scriptsize] at (2.6, 0.32) {Capa previa};
            \end{scope}
...
\begin{frame}{Antecedentes --- análisis de literatura}
    {\footnotesize Revisión sistemática PRISMA 2020--2025 presentada como póster en \textbf{LARS/LARC 2026} (\cite{Trevino2025_LARSPoster}).\par}
    \vspace{0.15cm}
...
\begin{frame}[fragile]{Sistema multicuerpo}
    \begin{columns}
        \column{.45\textwidth}
        Modelo propuesto en \cite{Al-Rawashdeh2022_Caracterization}: un marco base que vincula tres cadenas seriales mediante flexibilidades estructurales.
        \vspace{0.2cm}
        \begin{itemize}
            \item \textcolor{reticleEdge}{\textbf{Retícula ($j{=}1$)}}: desliza la fotomáscara.
            \item \textcolor{opticsEdge}{\textbf{Óptica ($j{=}2$)}}: lente y marco de metrología.
            \item \textcolor{waferEdge}{\textbf{Oblea ($j{=}3$)}}: desliza la oblea bajo el lente.
        \end{itemize}
        \vspace{0.15cm}
        {\footnotesize Cadenas móviles ($j{=}1,3$): \textbf{dos etapas} — carrera larga (LS) + Lorentz de banda ancha (SS).}

        \column{.55\textwidth}
        \centering
        \begin{tikzpicture}[scale=0.95, transform shape, every node/.style={font=\footnotesize},
            spring/.style={decorate, decoration={zigzag, segment length=3.5pt, amplitude=2.2pt, pre length=2pt, post length=2pt}, line width=0.55pt, mnnwInk},
            stage/.pic={
                \fill[#1!25] (-1.05,-0.32) rectangle (1.05,0.32);
                \draw[#1, line width=0.9pt] (-1.05,-0.32) rectangle (1.05,0.32);
            }
        ]
            % Floor (hatched)
            \fill[pattern=north east lines, pattern color=mnnwMute!60] (0,-0.25) rectangle (7.0,0);
            \draw[line width=0.6pt, mnnwInk] (0,0) -- (7.0,0);

            % Base frame: tall column + small foot on the floor
            \fill[frameFill, draw=frameEdge, line width=0.6pt]
                (0.25,0) rectangle (1.05,5.9);
            \node[rotate=90, font=\scriptsize, text=frameEdge] at (0.65,2.95) {marco base $\mathcal{F}_1$};

            % Light source (top)
            \fill[kit-yellow30, draw=kit-yellow!70!black, line width=0.5pt] (3.5,5.5) rectangle (4.7,6.0);
            \node[font=\scriptsize] at (4.1,5.75) {fuente};

            % Reticle row (top - moves left)
            \draw[spring] (1.05,4.55) -- (3.05,4.55);
            \node[font=\scriptsize, text=frameEdge, anchor=south] at (2.05,4.62) {$K_1, C_1$};
            \pic at (4.1,4.55) {stage=reticleEdge};
            \node[text=reticleEdge, font=\small\bfseries] at (4.1,4.55) {retícula};
            \draw[Stealth-, line width=1.3pt, kit-red100] (3.2,5.15) -- (5.0,5.15);
            \node[font=\scriptsize, text=kit-red100, anchor=south] at (4.1,5.15) {$\mathbf{v}_r$\,(\,$4\times$\,)};

            % Optics row (middle - static)
            \draw[spring] (1.05,2.85) -- (3.05,2.85);
            \node[font=\scriptsize, text=frameEdge, anchor=south] at (2.05,2.92) {$K_2, C_2$};
            \pic at (4.1,2.85) {stage=opticsEdge};
            \node[text=opticsEdge, font=\small\bfseries] at (4.1,2.85) {óptica};
            \node[font=\scriptsize, text=mnnwMute, anchor=west] at (5.25,2.85) {(fija)};

            % Wafer row (bottom - moves right)
            \draw[spring] (1.05,1.15) -- (3.05,1.15);
            \node[font=\scriptsize, text=frameEdge, anchor=south] at (2.05,1.22) {$K_3, C_3$};
            \pic at (4.1,1.15) {stage=waferEdge};
            \node[text=waferEdge, font=\small\bfseries] at (4.1,1.15) {oblea};
            \draw[-Stealth, line width=1.3pt, kit-blue100] (3.2,0.55) -- (5.0,0.55);
            \node[font=\scriptsize, text=kit-blue100, anchor=north] at (4.1,0.55) {$\mathbf{v}_w$};

            % Light beam from source through reticle, optics, to wafer
            \draw[draw=kit-yellow!85!black, line width=1.2pt, dashed] (4.1,5.5) -- (4.1,1.47);
            \node[font=\scriptsize, text=kit-yellow!55!black, anchor=west] at (4.85,4.05) {haz};

            % Sync bracket (right)
            \draw[decorate, decoration={brace, amplitude=4pt, mirror}, line width=0.7pt]
                (6.0,1.15) -- (6.0,4.55);
            \node[align=left, font=\scriptsize, anchor=west] at (6.2,2.85)
                {sincronía\\sub-nm};
        \end{tikzpicture}
    \end{columns}
\end{frame}
...
\begin{frame}{Simulación --- resultados}
    \begin{columns}
        ...
        {\scriptsize Izq.: error de seguimiento. \quad Der.: MA y MSD móviles.}
    \end{columns}
\end{frame}
...
\begin{frame}[fragile]{Conexión trayectoria $\leftrightarrow$ defecto}
    \begin{center}
        \begin{tikzpicture}[
            ...
            stepbox/.style={draw=mnnwInk, line width=0.6pt, rounded corners=2pt, minimum width=1.7cm, minimum height=0.9cm, align=center, fill=white},
            unknownbox/.style={draw=kit-red100, line width=0.9pt, dashed, rounded corners=2pt, minimum width=1.7cm, minimum height=0.9cm, align=center, fill=kit-red!8},
            solbox/.style={draw=opticsEdge, line width=0.6pt, rounded corners=2pt, minimum width=5.4cm, minimum height=1.0cm, align=center, fill=opticsFlex},
            ...
            \node[font=\scriptsize\bfseries, text=kit-red100, anchor=south, fill=white, inner sep=1pt] at ([yshift=8pt]gap.north) {brecha};

            \node[solbox] (sa) at (-1.5,-2.4) {\textbf{Física --- \textit{dose mapping}}\\\scriptsize MA/MSD locales $\to$ variación de dosis $\to$ umbral};
            \node[solbox, right=0.5cm of sa] (sb) {\textbf{Datos --- \textit{paired learning}}\\\scriptsize pares (error, mapa) $\to$ generador condicional aprendido};
        ...
        \begin{center}
          ...
          \begin{tikzpicture}[
            ...
            \begin{scope}[shift={(0.25, 1.35)}]
              \draw[square]  (0, 0.55) -- (0.35, 0.55) node[right, font=\scriptsize] {Cuadrado};
              \draw[fillet]  (0, 0.27) -- (0.35, 0.27) node[right, font=\scriptsize] {Bisel};
              \draw[meander] (0, 0.00) -- (0.35, 0.00) node[right, font=\scriptsize] {Meandro};
            \end{scope}
