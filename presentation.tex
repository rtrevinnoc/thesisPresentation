...
\newcommand{\UANLtitleframe}{%
  \begin{frame}[plain]
    ...
    \begin{textblock*}{\paperwidth}(2mm,60mm)
      \centering\includegraphics[height=30mm]{logos/duv_stepper_working.png}%
    \end{textblock*}%
  \end{frame}
}
...
\begin{frame}[t,fragile]{Antecedentes --- análisis de literatura}
    ...
    \begin{tikzpicture}[every node/.style={font=\small}]
        ...
        \foreach \name/\cnt/\col [count=\i] in {%
            ...
            \node[anchor=east, font=\small] at (0,\y) {\name};
            \fill[\col] (0.15,\y-\barH/2) rectangle ({0.15+\cnt*\unit},\y+\barH/2);
            \node[anchor=west, font=\small, text=mnnwInk] at ({0.22+\cnt*\unit},\y) {\cnt};
        }
        \node[anchor=west, font=\footnotesize, text=mnnwMute] at (0.15,-3.95) {reportes incluidos (n{=}12)};
    \end{tikzpicture}
    \end{center}
    \vspace{0.1cm}
    \begin{purpleblock}{Brecha}
        Métricas \textbf{servo-mecánicas} (MSE) sin considerar el impacto \textbf{morfológico} en el \textit{yield}.
    \end{purpleblock}
\end{frame}
...
\begin{frame}[fragile]{Sistema multicuerpo}
    \begin{center}
        \begin{tikzpicture}[scale=0.55, every node/.style={font=\footnotesize}]
            ...
            % ====== PROJECTION LENS ======
            \fill[opticsFlex!90, draw=opticsEdge, line width=1.0pt]
                (5.7,3.70) -- (5.4,5.70) -- (8.6,5.70) -- (8.3,3.70) -- cycle;
            \draw[opticsEdge, line width=0.6pt, fill=white] (7.0,5.20) ellipse (1.20 and 0.20);
            \node[text=opticsEdge, font=\footnotesize\bfseries, anchor=west] at (8.75,4.95) {lente};
            \node[text=opticsEdge, font=\footnotesize, anchor=west] at (8.75,4.55) {$4\times$};
            % light cone
            \fill[kit-yellow!55, draw=kit-yellow!75!black, line width=0.3pt, opacity=0.55]
                (5.75,5.70) -- (8.25,5.70) -- (7.18,3.80) -- (6.82,3.80) -- cycle;
            \fill[kit-yellow!60, draw=kit-yellow!75!black, line width=0.3pt, opacity=0.85]
                (6.82,3.70) -- (7.18,3.70) -- (7.12,3.15) -- (6.88,3.15) -- cycle;
...
\begin{frame}{Simulación --- resultados}
    ...
    \vspace{0.15cm}
    \begin{center}
        \includegraphics[width=0.47\linewidth,height=0.62\textheight,keepaspectratio]{pictures/sim_tracking.png}\hfill%
        \includegraphics[width=0.47\linewidth,height=0.62\textheight,keepaspectratio]{pictures/sim_ma_msd.png}\\[0.15cm]
        {\small Error de seguimiento \quad$|$\quad MA y MSD móviles}
    \end{center}
\end{frame}
...
\begin{frame}{Mapas de defectos y CNN}
    ...
    \begin{columns}[T]
        ...
        \column{.36\textwidth}
        \centering
        \includegraphics[width=\linewidth,height=0.65\textheight,keepaspectratio]{pictures/wm811k_samples.png}\\[0.10cm]
        {\small ejemplos por clase}

        \column{.38\textwidth}
        \centering
        \includegraphics[width=\linewidth,height=0.65\textheight,keepaspectratio]{pictures/cnn_confmat.png}\\[0.10cm]
        {\small matriz de confusión}
    \end{columns}
\end{frame}
