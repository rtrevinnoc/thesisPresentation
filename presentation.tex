...
% --- Shared TikZ styles for multi-body chain schematics ---
\tikzset{
...
% --- Per-chain figures for the appendix (split from the original combined schematic) ---
\newcommand{\standaloneFloorAndBase}[3]{%
  % #1: chain accent color   #2: shelf label   #3: axis top y
  \def\xLeft{-1.6}\def\xRight{7.5}
  \def\yBase{1.4}
  % Floor F_0
  \filldraw[hatchfloor, draw=mnnwInk, line width=0.7pt]
    (\xRight, 0) -- (\xLeft, 0) -- (\xLeft, -0.5) -- (\xRight, -0.5) -- cycle;
  \node[lbl, anchor=west] at (\xRight+0.08,0.20) {$\mathcal{F}_0$};
  \draw[axis] (0,-0.05) -- (0,#3);
  % Base frame F_1 shelf
  \filldraw[draw=frameEdge, line width=0.6pt, fill=frameFill]
    (\xLeft+0.2, \yBase-0.4) rectangle (\xRight-0.2, \yBase+0.4);
  \node[font=\small\bfseries, text=mnnwInk] at ({(\xLeft+\xRight)/2}, \yBase) {#2 $\;(\mathcal{F}_1)$};
  % Vertical mount between F_0 (y=0) and bottom of Marco Base (y=1.0)
  \draw[vsp] (1.20, 0) -- (1.20, 1.0);
  \node[lbl, anchor=east] at (1.15, 0.50) {$K_1$};
  \vdamperLong{2.00}{0}{1.0}
  \node[lbl, anchor=west] at (2.10, 0.50) {$C_1$};
}

\newcommand{\waferChainFigure}{%
  \begin{tikzpicture}[scale=0.42, transform shape, every node/.style={font=\small}]
    \def\yWa{3.4}\def\yWb{5.4}\def\yWc{7.4}\def\yWd{9.4}\def\yWe{11.4}\def\yWf{13.4}
    \standaloneFloorAndBase{waferEdge}{Marco Base}{14.0}
    ...
    \draw[line width=0.4pt, waferEdge] (6.9,\yWa-0.30) -- (6.9,\yWf+0.30);
    \node[font=\scriptsize\bfseries, text=waferEdge, rotate=-90, anchor=south]
      at (7.12,{(\yWa+\yWf)/2}) {Cadena de la oblea $(j{=}3)$};
  \end{tikzpicture}%
}

\newcommand{\opticsChainFigure}{%
  \begin{tikzpicture}[scale=0.7, transform shape, every node/.style={font=\small}]
    \def\yOa{3.4}\def\yOb{5.4}\def\yOc{7.4}
    \standaloneFloorAndBase{opticsEdge}{Marco Base}{8.0}
    \pgfmathsetmacro{\shelfO}{\yBase+0.4}
    ...
    \chainrow{\yOc}{oblk}{1.8}{ 2.7}{Lente $(N_2{=}4)$}{$K_{N_2,2}$}{$C_{N_2,2}$}{$f_{N_2,2}$}
    \draw[line width=0.4pt, opticsEdge] (4.6,\yOa-0.30) -- (4.6,\yOc+0.30);
    \node[font=\scriptsize\bfseries, text=opticsEdge, rotate=-90, anchor=south]
      at (4.82,{(\yOa+\yOc)/2}) {Cadena de óptica $(j{=}2)$};
  \end{tikzpicture}%
}

\newcommand{\reticleChainFigure}{%
  \begin{tikzpicture}[scale=0.42, transform shape, every node/.style={font=\small}]
    \def\yRa{3.4}\def\yRb{5.4}\def\yRc{7.4}\def\yRd{9.4}\def\yRe{11.4}\def\yRf{13.4}
    \standaloneFloorAndBase{reticleEdge}{Marco Base}{14.0}
    \pgfmathsetmacro{\shelfR}{\yBase+0.4}
    ...
    \draw[line width=0.4pt, reticleEdge] (7.30,\yRa-0.30) -- (7.30,\yRf+0.30);
    \node[font=\scriptsize\bfseries, text=reticleEdge, rotate=-90, anchor=south]
      at (7.52,{(\yRa+\yRf)/2}) {Cadena de la retícula $(j{=}1)$};
  \end{tikzpicture}%
}
    ...
    \begin{columns}[T]
        ...
        \begin{tikzpicture}[scale=0.68, every node/.style={font=\scriptsize}]
            ...
            \begin{scope}[shift={(0.15,-2.50)}]
                \draw[mnnwInk,   line width=1.3pt] (0,0.45)  -- (0.45,0.45);
                \node[right] at (0.45,0.45)  {Cuadrada};
                \draw[mnnwSignal,line width=1.3pt] (0,0.00)  -- (0.45,0.00);
                \node[right] at (0.45,0.00)  {Biselada};
                \draw[opticsEdge,line width=1.3pt] (0,-0.45) -- (0.45,-0.45);
                \node[right] at (0.45,-0.45) {Alto Orden};

                \node[right,text=mnnwMute] at (2.85, 0.45)  {\textit{acceleración}\,$\to\infty$};
                \node[right,text=mnnwMute] at (2.85, 0.00)  {\textit{jerk}\,$\to\infty$};
                \node[right,text=kit-lightgreen100!70!black] at (2.85,-0.45)
                    {\textit{snap} acotado};
            \end{scope}
        \end{tikzpicture}
        \end{center}
        ...
        \begin{center}
          ...
          \end{tikzpicture}
            ...
            \vspace{0.05cm}
            {\footnotesize$\displaystyle p(t)=p_0+v_0\,t+\tfrac{a_0}{2}\,t^{2}+\tfrac{j_0}{6}\,t^{3}+\tfrac{s_{\max}}{24}\,t^{4}$}
        \end{center}
    \end{columns}
\end{frame}
...
\begin{frame}[fragile]{Siguientes pasos}
    ...
    \begin{tikzpicture}[
        ...
        \node[stp, right=0.55cm of b] (c) {\textbf{3.}\\Comparar trayectorias suaves\\en la literatura actual\\\footnotesize\cite{Al-Rawashdeh2023_Trajectories}};
        \draw[ar] (a) -- (b);
        \draw[ar] (b) -- (c);
    \end{tikzpicture}
    \end{center}
...
\begin{frame}[fragile]{Apéndice: cadena de la oblea}
    \begin{center}
        \waferChainFigure
    \end{center}
\end{frame}

\begin{frame}[fragile]{Apéndice: cadena de óptica}
    \begin{center}
        \opticsChainFigure
    \end{center}
\end{frame}

\begin{frame}[fragile]{Apéndice: cadena de la retícula}
    \begin{center}
        \reticleChainFigure
    \end{center}
\end{frame}

\begin{frame}{Apéndice: control PID}
    \vspace{0.10cm}
    \begin{center}
    \begin{equation*}
    ...
    \begin{columns}[T,onlytextwidth]
        ...
        \centering\small\textbf{Ganancias}\\[0.18cm]
        \footnotesize
        \begin{tabular}{r@{$\;\approx\;$}l}
            $K_p$ & $8.66\times 10^{6}$ N/m \\[2pt]
            $K_i$ & $2.72\times 10^{8}$ N/(m·s) \\[2pt]
            $K_d$ & $2.75\times 10^{4}$ N·s/m \\[2pt]
            $m$   & $87$ kg \\
        \end{tabular}
        \end{column}
        \begin{column}{0.51\textwidth}
        \centering\small\textbf{Derivación (\cite{Butler2011_PositionControl})}\\[0.18cm]
        \footnotesize
        Ancho de banda $f_{bw}=50$ Hz, $\omega_c = 2\pi f_{bw}$, $\alpha\approx 3$.\\[0.15cm]
        \begin{tabular}{r@{$\;=\;$}l@{$\;\Rightarrow\;$}l}
            $K_p$ & $m\,\omega_c^2$               & $m\,(2\pi f_{bw})^2$ \\[2pt]
            $K_d$ & $K_p / (2\pi f_d)$            & $m\,\omega_c$ \\[2pt]
            $K_i$ & $K_p\cdot 2\pi f_i$           & $K_p\,\omega_c/\alpha^2$ \\
        \end{tabular}\\[0.15cm]
        \scriptsize\itshape con $f_d = f_{bw}/\alpha$,\; $f_i = f_{bw}/\alpha^2$
        \end{column}
    \end{columns}
\end{frame}
\backupend
